...
CREATE TABLE CLIENTESREGISTRADOS(
    ...
    CONSTRAINT clien_ck_usr CHECK ( LENGTH(USUARIO) > 3),
    CONSTRAINT clien_ck_cntrs CHECK ( LENGTH(CONTRASEÑA) > 5),
    CONSTRAINT clien_ck_rol CHECK ( LENGTH(ROL) > 3),
    CONSTRAINT clien_ck_correo CHECK ( LENGTH(CORREO) > 4));
